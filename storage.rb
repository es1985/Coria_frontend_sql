...
module Storage
  ...
  def self.get_meta_data(nodeid)
    con = Mysql.new '127.0.0.1', 'root', 'C0riA','as_dw'
    query = "SELECT * FROM as_dw.AS WHERE as_dw.AS.id=#{nodeid}"
    print(query)
    rs = con.query(query)  
    if rs.num_rows > 0 
      res1 = rs.fetch_row  
